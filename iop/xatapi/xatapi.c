...
int atapi_check_if_drive_ready(int check_nowait)
{
  ...
  ata_ctrl_mask_tmp = ata_ctrl_tmp & 0xC0;
  if ( !check_nowait )
  {
    while ( 1 )
    {
      if ( ata_ctrl_mask_tmp != 64 )
      {
        do
        {
          ...
          DelayThread(2000);
          ata_ctrl_tmp2 = xatapi_12_get_ata_control();
          ata_ctrl_tmp = ata_ctrl_tmp2;
        }
        while ( (ata_ctrl_tmp2 & 0xC0) != 64 );
      }
...
int SpdDmaTransfer(unsigned int device, void *buf, u32 bcr_in, int dir)
{
  ...
  dmac_ch_get_chcr(3u);
  device_1 = device;
  if ( device < 2 || (result = -1, g_dev5_predma_cbs[device_1]) && (result = -1, g_dev5_postdma_cbs[device_1]) )
  {
    if ( g_xatapi_verbose > 0 )
      Kprintf("Wait Intr\n");
...
int speed_init(void)
{
  ...
  {
    ++i2;
    g_dev5_predma_cbs[i2x] = 0;
    g_dev5_postdma_cbs[i2x] = 0;
    i2x = i2;
  }
  while ( i2 < 4 );
  speedLEDCtl(0);
      ...
      do
      {
        ...
        do
        {
          ...
          if ( *ptr_ptr >= 0 && (look_ctype_table(*ptr_ptr) & 0x17) != 0 )
            ...
            *chrbuf_ptr_ptr_ptr = '.';
          curbuf_cur = ++curbyte & 0xF;
        }
        while ( curbyte < curline );
        ++totalx;
        len -= curline;
        offstotal += curline;
      }
...
LABEL_18:
      ...
      else if ( command >= 0xA2u || command < 0xA0u )
      {
        goto LABEL_18;
      }
    }
        ...
        u32 unk10)
{
  ...
  WaitEventFlag(g_acmd_evfid, 1u, 16, efbits);
  result = sceAtaExecCmd(buf, blkcount, feature, nsector, sector, lcyl, hcyl, select, command, unk10);
  retres = result;
  if ( result )
  {
    SetEventFlag(g_acmd_evfid, 1u);
        ...
        int proto)
{
  ...
  switch ( proto )
  {
    ...
    case 8:
      ...
      goto LABEL_42;
    case 4:
      if ( pkt_scsi_cmd < 0x53u )
      {
        if ( pkt_scsi_cmd < 0x51u && pkt_scsi_cmd != 0x28 )
        {
          if ( pkt_scsi_cmd >= 0x29u )
          {
            if ( pkt_scsi_cmd >= 0x45u )
            {
              if ( pkt_scsi_cmd != 0x46 && pkt_scsi_cmd != 0x4A )
                goto LABEL_38;
            }
            else if ( pkt_scsi_cmd < 0x42u && pkt_scsi_cmd != 0x3C )
            {
              goto LABEL_38;
            }
          }
          else if ( pkt_scsi_cmd != 0x12 )
          {
            if ( pkt_scsi_cmd >= 0x13u )
            {
              if ( pkt_scsi_cmd != 0x23 && pkt_scsi_cmd != 0x25 )
                goto LABEL_38;
            }
            else if ( pkt_scsi_cmd != 3 )
            {
              goto LABEL_38;
            }
          }
...
LABEL_37:
      ...
      if ( pkt_scsi_cmd < 0xAEu )
      {
        if ( pkt_scsi_cmd < 0xACu && pkt_scsi_cmd != 0xA4 )
        {
          if ( pkt_scsi_cmd >= 0xA5u )
          {
            if ( pkt_scsi_cmd != 0xA8 )
              goto LABEL_38;
          }
          else if ( pkt_scsi_cmd != 0x5A )
          {
            goto LABEL_38;
          }
        }
      ...
      if ( pkt_scsi_cmd < 0xBFu )
      {
        if ( pkt_scsi_cmd < 0xBDu && pkt_scsi_cmd != 0xB9 )
          goto LABEL_38;
        goto LABEL_37;
      }
      if ( pkt_scsi_cmd == 0xDA )
        goto LABEL_37;
LABEL_38:
...
LABEL_42:
        ...
        || (USec2SysClock(0x93D1CC0u, &sysclk), result = SetAlarm(&sysclk, AtaAlarmrHandle, 0), result >= 0) )
      {
        ...
        if ( pkt_len_div2 )
        {
          ...
          while ( pkt_data_i < pkt_len_div2_tmp );
        }
        if ( g_xatapi_verbose > 0 )
...
LABEL_14:
      tmp_nsec = nsec;
      return sceCdAtapiExecCmd_local(tmp_n, tmp_buf, tmp_nsec, secsize, pkt, pkt_len, proto);
    }
...
int xatapi_7_sceCdAtapiExecCmd(s16 n, void *buf, int nsec, int secsize, void *pkt, int pkt_len, int proto)
{
  ...
  WaitEventFlag(g_acmd_evfid, 1u, 16, efbits);
  result = sceCdAtapiExecCmd(n, buf, nsec, secsize, pkt, pkt_len, proto);
  retres = result;
  if ( result )
  {
    SetEventFlag(g_acmd_evfid, 1u);
...
int IoRun_atapi(ata_cmd_state_t *cmd_state)
{
  ...
  if ( result >= 0 )
  {
    blktotal = cmd_state->blkcount_atapi * cmd_state->blksize_atapi;
    result = 0;
    if ( blktotal )
    {
      while ( 1 )
...
LABEL_21:
  ...
  if ( blkremainder )
  {
    spd_intr_stat_2 = dev5_speed_regs.r_spd_intr_stat;
    if ( (spd_intr_stat_2 & 1) == 0 )
    {
...
int DmaRun_atapi_extrans1(char *buf, int blkcount, int blksize, int dir)
{
  ...
  FpgaClearBuffer();
  FpgaXfdir(dir);
  blksectors = (unsigned int)(blkcount * blksize) >> 9;
...
int DmaRun_spck(char *buf, unsigned int secsize)
{
  ...
  if ( secsize >> 9 )
  {
    ...
    {
      do
        unk8148_val = do_fpga_check_unk8148();
      while ( unk8148_val < 4 );
...
LABEL_31:
  ...
  if ( atad_cmd_state.type == 4 || atad_cmd_state.type == 6 || atad_cmd_state.type == 5 )
  {
...
int sceCdAtapiWaitResult_local(void)
{
  ...
  int dmares; // $v0
  char *buf_atapi; // $a0
  int blkcount_atapi; // $a1
  int blksize_atapi; // $a2
  int dir_atapi; // $a3
  int trycnt1; // $v1
  ...
  if ( atad_cmd_state.type_atapi != 4 && atad_cmd_state.type_atapi != 6 && atad_cmd_state.type_atapi != 5 )
  {
  ...
  if ( !g_reset_scrambling_pack )
  {
    buf_atapi = (char *)atad_cmd_state.buf_atapi;
    blkcount_atapi = atad_cmd_state.blkcount_atapi;
    blksize_atapi = atad_cmd_state.blksize_atapi;
    dir_atapi = atad_cmd_state.dir_atapi;
LABEL_29:
    res = DmaRun_atapi(buf_atapi, blkcount_atapi, blksize_atapi, dir_atapi);
    goto LABEL_30;
  }
  if ( atad_cmd_state.dir_atapi )
  {
    ...
    if ( (int)(atad_cmd_state.blkcount_atapi * atad_cmd_state.blksize_atapi) > 0 )
    {
      ...
      while ( 1 )
      {
        ...
        blktotal2 = blktotal1 - blkoffs;
        if ( blkoffs >= blktotal1 )
          goto LABEL_30;
      }
      buf_atapi = (char *)atad_cmd_state.buf_atapi;
      blkcount_atapi = atad_cmd_state.blkcount_atapi;
...
void FpgaClearBuffer(void)
{
  ...
  do
  {
    while ( dev5_fpga_regs.r_fpga_exbufe )
      ;
  }
  while ( dev5_fpga_regs.r_fpga_sl3bufe );
  ...
  else
  {
    ...
    *retptr = 0;
    ...
    if ( g_xatapi_verbose > 0 )
    {
      ...
      if ( bufchk )
        strtmp = "RDI";
      Kprintf("%s():out %d %s Pack\n", "Mpeg2CheckPadding", *retptr, strtmp);
      return bufchk;
    }
  }
  return result;
