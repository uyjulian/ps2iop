...
int g_TimeOut2 = 5000;
iop_sys_clock_t g_readid_systemtime = { 0, 0 }; // weak
...
void __fastcall cdvdman_termcall(int with_stop)
{
	...
	sceCdDecSet(0, 0, 0);
	if ( (dmac_ch_get_chcr(3) & 0x1000000) != 0 )
	{
...
int __cdecl cdvdman_devready()
{
	...
	for ( i = 0; i < 100; i += 1 )
	{
		...
		if ( (dev5_regs.m_dev5_reg_005 & 0xC0) == 0x40 && !g_cdvdman_istruct.m_read2_flag && !g_cdvdman_strm_id )
		{
			return 1;
		}
		DelayThread(10000);
...
int __cdecl cdvd_odcinit(CDVDMAN_FILEDATA *fh, int open_or_close, int id)
{
	...
	if ( open_or_close )
	{
		...
		cache_result = open(cache_filename, 0x603, 0x1ff);
		...
		if ( cache_result >= 0 )
		{
			...
			{
				cache_result = ioctl2(cache_file_fd_new, 0x7001, &ioctl_arg, 4, 0, 0);
			}
		}
		if ( cache_result >= 0 )
		{
...
int __cdecl cdvdman_cache_invalidate(CDVDMAN_FILEDATA *filedata, int index)
{
	...
	if ( fileio_res >= 0 )
	{
		...
		{
			fileio_res = write(filedata->m_cache_file_fd, filedata->m_fd_rcvbuf, 0x800);
			if ( fileio_res < 0 )
			{
				break;
			}
...
int __fastcall cdrom_internal_cache_read(const iop_file_t *f, int nbytes)
{
	...
	if ( filedata->m_cluster_cur == -1 )
	{
		...
		{
			filedata->m_cluster_cur = (filedata->m_read_pos >> 14) & ~0x7;
...
int __fastcall cdrom_internal_write_cache(const iop_file_t *f, unsigned int nbytes)
{
	...
	int tray_open; // $s0
	int Error; // $a1
	sceCdRMode rmode; // [sp+18h] [-8h] BYREF
...
int __cdecl cdrom_open(iop_file_t *f, const char *name, int mode, int arg4)
{
	...
	filedata->m_filemode = mode & ~0x40000000;
...
int __fastcall cdrom_internal_read(const iop_file_t *f, char *addr, int nbytes)
{
	...
	int Error; // $v0
	sceCdRMode rmode; // [sp+10h] [-28h] BYREF
	int i; // [sp+18h] [-20h]
	...
	for ( i = 0; i < nbytes; i += nbytes_segment )
	{
		...
		if ( nbytes_cur <= 0x4000 )
		{
			...
			nbytes_div_2048 = (nbytes_cur / 0x800) + (( (nbytes_cur & 0x7FF) != 0 ) ? 1 : 0);
				...
				unsigned int buflen)
{
	...
	switch ( cmd )
	{
		...
		case 0x4390:
			...
			break;
		case 0x4391:
			*(_DWORD *)bufp = sceCdSC(0xFFFFFFF5, &on_dual_tmp);
			break;
		case 0x4392:
		...
		case CDIOC_FSCACHEDELETE:
			...
			retval2 = (i == (sizeof(g_cdvdman_handles)/sizeof(g_cdvdman_handles[0]))) ? path_tbl_init(0, 0, 0) : -16;
			break;
		default:
			retval2 = -5;
			break;
...
int __cdecl cdrom_lseek(iop_file_t *f, int offset, int pos)
{
	...
	if ( (filedata->m_fd_flags & 8) != 0 )
	{
		...
		if ( !sceCdStSeekF(filedata->m_file_lsn + retval / 0x800) )
		{
			retval = -5;
		}
	}
...
int __cdecl sceCdLayerSearchFile(sceCdlFILE *fp, const char *path, int layer)
{
	...
	if ( PollEventFlag(g_sfile_evid, 1, 0x10, &efbits) == -421 )
	{
		return 0;
	}
...
int __cdecl disc_read(int size, int loc, void *buffer, int layer)
{
	int f; // $s6
	int i; // $s0
	sceCdRMode rmode; // [sp+20h] [-8h] BYREF
	int has_success;
	...
	switch ( g_cdvdman_srchspd )
	{
		...
		case 0:
		case 1:
			rmode.spindlctrl = g_cdvdman_srchspd != 0;
			break;
		default:
			rmode.spindlctrl = 1;
			break;
...
int __cdecl path_tbl_init(u32 blocks, char *fname, int action)
{
	...
	{
		CpuSuspendIntr(&state);
		g_cdvdman_pathtbl = (CDVDMAN_PATHTBL_T *)AllocSysMemory(1, sizeof(CDVDMAN_PATHTBL_T) * blocks, 0);
		...
		if ( v >= 0 )
		{
			...
			{
				blocksbs = blocks << 11;
				ioctl2(g_cache_path_fd, 0x7001, &blocksbs, 4, 0, 0);
			}
...
int __cdecl cdvdman_initcfg()
{
	...
	for ( i = 0; i <= 100; i += 1 )
	{
		...
		if ( !sceCdGetMVersion(m_version, &eflag) && (eflag & 0x80) != 0 )
		{
			vDelayThread(2000);
...
// 406B90: using guessed type u32 rdto;
...
sceCdCBFunc __cdecl sceCdCallback(sceCdCBFunc function)
{
	...
	if ( sceCdSync(1) != 0 || PollEventFlag(g_ncmd_evid, 1, 0x10, &efbits) == -421 )
	{
		return 0;
	}
...
u32 __cdecl sceCdLsnDualChg(u32 lsn)
{
	...
	if ( cdvdman_isdvd() != 0 && DvdDual_infochk() != 0 )
	{
		if ( g_cdvdman_istruct.m_dual_layer_emulation )
		{
			...
			{
				u32 traychk; // [sp+20h] [-8h] BYREF

				VERBOSE_KPRINTF(0, "CDVD: Exchange it for the Layer_%d_Disk Please.\n", layer_disk_needed);
				while ( g_cdvdman_minver_60200 ? !cdvdman_scmd_sender_3B(0) : !sceCdTrayReq(0, &traychk) )
				{
					;
				}
				cdrmode.trycount = 0;
		...
		{
			change_lsn = lsn - (( g_cdvdman_istruct.m_opo_or_para && (lsn >= (u32)g_cdvdman_istruct.m_layer_1_lsn) ) ? 0x10 : 0);
			has_change_lsn = 1;
		}
	}
	if ( has_change_lsn )
...
int __cdecl sceCdReadDvdDualInfo(int *on_dual, unsigned int *layer1_start)
{
	...
	*layer1_start = 0;
	...
	if ( g_cdvdman_mmode != 2 && g_cdvdman_mmode != 0xFF )
	{
		return 0;
	}
	cdrmode.trycount = 0;
	...
	switch ( code )
	{
		...
		case 0xFFFFFFF4:
			...
			switch ( sceCdGetDiskType() )
			{
				...
				case 0x12:
				case 0x13:
					return g_cdvdman_mmode == 1 || g_cdvdman_mmode == 0xFF;
				case 0x14:
					return g_cdvdman_mmode == 2 || g_cdvdman_mmode == 0xFF;
				case 0xFD:
...
int __cdecl set_prev_command(int cmd, const char *sdata, int sdlen, char *rdata, int rdlen, int check_sef)
{
	...
	if ( check_sef == 1 && PollEventFlag(g_scmd_evid, 1, 0x10, &efbits) == -421 )
	{
		return 0;
	}
	...
	{
		while ( (dmac_ch_get_chcr(3) & 0x1000000) != 0 && !g_cdvdman_istruct.m_wait_flag )
		{
...
int sceCdBreak(void)
{
	...
	if ( PollEventFlag(g_ncmd_evid, 1, 0x10, &efbits) == -421 )
	{
		return 0;
	}
	CpuSuspendIntr(&state);
...
int __fastcall intrh_dma_3(cdvdman_internal_struct_t *s)
{
	...
	if ( dmacbres )
	{
		if ( s->m_dma3prm.m_dma3_msectors )
		{
			dmac_ch_set_chcr(3, 0);
			dmac_ch_get_chcr(3);
			dmac_ch_set_madr(3, (u32)s->m_dma3prm.m_dma3_maddress);
			dmac_ch_set_bcr(3, ((s->m_dma3prm.m_dma3_blkcount * s->m_dma3prm.m_dma3_csectors) << 16) | s->m_dma3prm.m_dma3_blkwords);
...
int __cdecl cdvdman_setdma3(DMA3PARAM *b18)
{
	...
	dmac_ch_set_madr(3, (u32)b18->m_dma3_maddress);
	dmac_ch_set_bcr(3, (b18->m_dma3_blkcount * (b18->m_dma3_csectors ? b18->m_dma3_csectors : 1)) << 16 | b18->m_dma3_blkwords);
	dmac_ch_set_chcr(3, 0x41000200);
...
int __cdecl cdvdman_send_ncmd(int ncmd, const void *ndata, int ndlen, int func, DMA3PARAM *b18, int check_cb)
{
	...
	if ( check_cb == 1 && PollEventFlag(g_ncmd_evid, 1, 0x10, &efbits) == -421 )
	{
		return -1;
	}
	if ( (dev5_regs.m_dev5_reg_005 & 0xC0) != 0x40
...
int __cdecl cdvdman_mediactl(int code)
{
	...
	rdata = 0;
	if ( PollEventFlag(g_scmd_evid, 1, 0x10, &efbits) == -421 )
	{
		return 0;
	}
...
int __cdecl sceCdRead0(u32 lsn, u32 sectors, void *buffer, sceCdRMode *mode, int csec, void *callback)
{
	...
	switch ( sceCdGetDiskType() )
	{
		...
		case 0x12:
		case 0x13:
			if ( g_cdvdman_mmode != 1 && g_cdvdman_mmode != 0xFF )
			{
				vSetEventFlag(g_ncmd_evid, 1);
				return 0;
			}
...
void __cdecl Read2intrCDVD(int read2_flag)
{
	...
	else if ( !g_cdvdman_istruct.m_interupt_read_state || g_cdvdman_istruct.m_cdvdman_csec )
	{
		...
		else
		{
			...
			{
				g_cdvdman_istruct.m_cdvdman_rsec = ((unsigned int)(g_cdvdman_istruct.m_cdvdman_csec + 0x40) < (unsigned int)g_cdvdman_istruct.m_cdvdman_nsec) ? 0x40 : (g_cdvdman_istruct.m_cdvdman_nsec - g_cdvdman_istruct.m_cdvdman_csec);
				if ( (( read2_flag == 3 ) ? sceCdRead0 : sceCdRV)(
...
int __cdecl sceCdRV(u32 lsn, u32 sectors, void *buf, sceCdRMode *mode, int arg5, void *cb)
{
	...
	u32 efbits; // [sp+40h] [-8h] BYREF

	// The following call to sceCdGetDiskType was inlined
	if ( sceCdGetDiskType() != 0x14 || (g_cdvdman_mmode != 2 && g_cdvdman_mmode != 0xFF) || (PollEventFlag(g_ncmd_evid, 1, 0x10, &efbits) == -421) )
	{
		return 0;
	}
...
int __cdecl sceCdPowerOff(u32 *result)
{
	...
	*result = 0;
	...
	if ( !g_cdvdman_minver_x_model_15 )
	{
		...
		{
			u32 traychk; // [sp+18h] [-8h] BYREF

			sceCdTrayReq(1, &traychk);
			vDelayThread(250000);
		}
	}
...
int __cdecl sceCdReadDiskID(unsigned int *id)
{
	...
	*(_BYTE *)id = 0;
	...
	sceCdSync(3);
	if ( cdvdman_ncmd_sender_0C(0, 0, 0x4B) == 0 )
	{
		return 0;
	}
	sceCdSync(3);
...
int __cdecl sceCdDoesUniqueKeyExist(u32 *status)
{
	int disktype_tmp; // $s0
	vu8 dev5_reg_038; // $s0
	sceCdRMode rmode; // [sp+18h] [-18h] BYREF
