...
int __cdecl sceCdSync(int mode)
{
	...
	switch ( mode )
	{
		case 0:
			...
			{
				WaitEventFlag(g_cdvdman_intr_efid, 1, 0, resbits);
			}
			break;
		case 1:
		...
		case 32:
			WaitEventFlag(g_cdvdman_intr_efid, 0x21, 1, resbits);
			...
			if ( (efinfo.currBits & 0x20) == 0 )
			{
				...
				{
					WaitEventFlag(g_cdvdman_intr_efid, 0x20, 0, resbits);
				}
			}
			break;
		default:
...
int __cdecl cdvdman_send_ncmd(int ncmd, const void *ndata, int ndlen, int func, DMA3PARAM *b18, int check_cb)
{
	...
	if ( QueryIntrContext() )
	{
		iClearEventFlag(g_cdvdman_intr_efid, ~0x1);
	}
	else
	{
		ClearEventFlag(g_cdvdman_intr_efid, ~0x1);
	}
	for ( i = 0; i < ndlen; i += 1 )
	{
...
int __cdecl sceCdRead(u32 lbn, u32 sectors, void *buffer, sceCdRMode *mode)
{
	...
	vClearEventFlag(g_fio_fsv_evid, ~0x1);
	CpuResumeIntr(state);
