...
int __cdecl cdrom_init(iop_device_t *dev)
{
	...
	int scres_unused; // [sp+20h] [-8h] BYREF
...
int __fastcall cdrom_internal_write_cache(const iop_file_t *f, unsigned int nbytes)
{
	...
	for ( i = rst; i <= cur; i += 1 )
	{
		...
		if ( (((int)filedata->m_fd_rcvbuf[(i - cluster_cur) >> 3] >> ((i - cluster_cur) & 7)) & 1) == 0 )
		{
			...
			{
				if ( (sceCdStatus() & SCECdStatShellOpen) != 0 )
				{
					g_cdvdman_iocache = 0;
					tray_open = 1;
					break;
				...
				unsigned int buflen)
{
	...
	switch ( cmd )
	{
		...
		case CDIOC_FSCACHEDELETE:
			...
			retval2 = (i == (sizeof(g_cdvdman_handles)/sizeof(g_cdvdman_handles[0]))) ? path_tbl_init(0, 0, 0) : -EBUSY;
			break;
		default:
			retval2 = -EIO;
			break;
...
int __cdecl sceCdSync(int mode)
{
	...
	switch ( mode )
	{
		...
		case 16:
			...
			break;
		case 17:
			return (!sceCdCheckCmd() || g_cdvdman_istruct.m_read2_flag || g_cdvdman_ee_rpc_fno || (g_cdvdman_istruct.m_stream_flag != 0)) ? 1 : 0;
		case 32:
...
int __cdecl sceCdSearchDir(char *dirname, int layer)
{
	...
	return sceCdLayerSearchFile(&fileinfo, dirname, layer) != 0 ? g_cdvdman_fs_cdsec : SCECdErREADCFR;
}
...
// 406B90: using guessed type u32 read_to;
...
sceCdCBFunc __cdecl sceCdCallback(sceCdCBFunc function)
{
	...
	if ( sceCdSync(1) != 0 || PollEventFlag(g_ncmd_evid, 1, 0x10, &efbits) == KE_EVF_COND )
	{
		return 0;
	}
	...
	if ( !ext_passthrough )
	{
		...
		if ( dev5_reg_013_masked != 0 )
		{
			if ( ((unsigned __int8)s->m_last_error == 48 || ((unsigned __int8)s->m_last_error == 1 && s->m_read_to))
				...
				&& !s->m_stream_flag
				&& g_cdvdman_cmdfunc != 9
				&& g_cdvdman_cmdfunc != SCECdFuncReadCDDA
...
void __cdecl cdvdman_init()
{
	...
	RegisterIntrHandler(IOP_IRQ_CDVD, 1, (int (__cdecl *)(void *))intrh_cdrom, &g_cdvdman_istruct);
...
int __cdecl cdvdman_read(u32 lsn, u32 sectors, void *buf, sceCdRMode *mode, int decflag, int shift, int ef1, int ef2)
{
	...
	int scres_unused; // [sp+1Ch] [-Ch] BYREF
	int dvd; // [sp+20h] [-8h]
...
int __cdecl sceCdPowerOff(u32 *result)
{
	...
	*result = 0;
	...
	{
		while ( (sceCdStatus() & SCECdStatShellOpen) != 0 )
		{
...
int __cdecl sceCdDoesUniqueKeyExist(u32 *status)
{
	...
	*status = 0;
	...
	if ( (sceCdStatus() & SCECdStatSpin) != 0 )
	{
		CpuResumeIntr(state);
	}
	else
